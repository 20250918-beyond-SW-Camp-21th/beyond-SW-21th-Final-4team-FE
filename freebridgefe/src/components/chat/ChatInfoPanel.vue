  ...
  <div class="h-full bg-slate-900 border-l border-white/5 flex flex-col overflow-y-auto custom-scrollbar">
    ...
    <div class="p-6 border-b border-white/5">
      <div class="flex items-center justify-between mb-4">
        <h3 class="text-sm font-bold text-white uppercase tracking-wider">현재 제안 프로젝트</h3>
      </div>
    ...
    <div class="p-6">
      <h3 class="text-sm font-bold text-white uppercase tracking-wider mb-4">공유 파일</h3>
      <div class="space-y-3">
        <div
...
const otherParticipantSummary = computed(() => {
  ...
  if (authStore.user.role === 'EMPLOYER') {
    ...
    if (freelancer?.bio?.trim()) {
      return freelancer.bio.trim();
    }
...
const handleProfileClick = () => {
  ...
  if (authStore.user.role === 'EMPLOYER') {
    ...
    freelancerProfile.value = {
      name: freelancer?.name || chatStore.getParticipantName(room, otherId) || '정보 없음',
      avatarUrl: freelancer?.avatar || null,
      job: freelancer?.jobTitle || freelancer?.job || null,
      careerYears: freelancer?.experience ?? null,
      wage: freelancer?.monthlySalary ?? null,
      grade: null,
      introduction: freelancer?.bio || otherParticipantSummary.value || null,
      skills: freelancer?.skills ?? [],
      portfolioUrl: freelancer?.portfolio ?? null,
      resumeUrl: null
    };
  ...
  employerProfile.value = {
    companyName: job?.employerName || proposal?.employerName || chatStore.getParticipantName(room, otherId) || '정보 없음',
    logoUrl: null,
    industry: null,
    location: null,
    phone: null,
    website: null,
    description: job?.description || proposal?.message || otherParticipantSummary.value || null
  };

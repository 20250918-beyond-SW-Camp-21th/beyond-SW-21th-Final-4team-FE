...
<template>
  <div v-if="isOpen && project" class="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" @click.self="$emit('close')">
    ...
    <div
        class="w-full max-w-4xl bg-white/5 border border-white/10 rounded-[32px] shadow-[0_30px_90px_-60px_rgba(255,255,255,0.35)] max-h-[90vh] overflow-y-auto custom-scrollbar backdrop-blur-2xl"
    >
      <!-- Header -->
      <div class="sticky top-0 z-10 flex items-center justify-between p-6 bg-black/50 backdrop-blur-xl border-b border-white/10">
        <div>
           <div class="flex items-center gap-3 mb-2">
               <span class="px-3 py-1.5 text-[11px] font-semibold uppercase tracking-[0.2em] border rounded-full" :class="getStatusBadge(project.status).class">
                   {{ getStatusBadge(project.status).label }}
               </span>
               <span class="text-xs text-white/40 font-mono tracking-wide">ID: {{ project.id }}</span>
           </div>
      ...
      <div class="p-8 space-y-8">
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
            <!-- Period -->
            <div class="p-5 border bg-white/5 rounded-2xl border-white/10">
                <div class="flex items-center gap-2 mb-2 text-xs text-white/50 uppercase tracking-[0.2em]">
                    <Calendar class="w-4 h-4 text-blue-300" />
                    프로젝트 기간
                </div>
                <div class="text-base font-semibold text-white">{{ project.startDate }} ~ {{ project.endDate }}</div>
            </div>
            <!-- Budget -->
            <div class="p-5 border bg-white/5 rounded-2xl border-white/10">
                <div class="flex items-center gap-2 mb-2 text-xs text-white/50 uppercase tracking-[0.2em]">
                    <DollarSign class="w-4 h-4 text-emerald-300" />
                    총 예산
                </div>
                <div class="text-base font-semibold text-white">{{ project.budget }}</div>
            </div>
            <!-- Progress -->
            <div class="p-5 border bg-white/5 rounded-2xl border-white/10">
                <div class="flex items-center gap-2 mb-2 text-xs text-white/50 uppercase tracking-[0.2em]">
                ...
                <div class="flex items-end gap-2">
                    <span class="text-xl font-semibold text-white">{{ project.progress }}%</span>
                    <div class="flex-1 h-2 mb-1 bg-white/5 rounded-full overflow-hidden">
                        <div class="h-full bg-gradient-to-r from-white/70 to-white/30 rounded-full" :style="{ width: `${project.progress}%` }"></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div>
            <h3 class="flex items-center gap-2 mb-4 text-base font-semibold text-white">
                <FileText class="w-5 h-5 text-indigo-300" />
                프로젝트 상세 내용
            </h3>
            <div class="p-6 leading-relaxed border bg-white/5 rounded-2xl border-white/10 text-sm text-white/80">
                {{ project.description }}
            </div>
        </div>
            ...
            <div class="bg-white/5 border border-white/10 rounded-2xl p-5">
                ...
                <div v-else class="flex flex-wrap gap-3">
                    <div
                      ...
                      :key="label"
                      class="px-3 py-2 rounded-full text-xs font-semibold text-white/80 bg-white/5 border border-white/10 flex items-center gap-2"
                    >
                      <Tag class="w-3.5 h-3.5 text-white/60" />
                      {{ label }}: {{ count }}명
                    </div>
                    <div class="px-3 py-2 rounded-full text-xs font-semibold text-white/80 bg-white/5 border border-white/10">
                      총 {{ totalApplicants }}명
                    </div>
                </div>
            </div>

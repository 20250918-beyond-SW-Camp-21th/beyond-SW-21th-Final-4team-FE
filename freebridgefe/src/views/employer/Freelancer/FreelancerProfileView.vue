...
import { 
    User, Mail, Briefcase, Star, Clock, 
...
<template>
    <div v-if="freelancer" class="max-w-[1400px] mx-auto px-4 md:px-8 py-12 text-white font-sans">
        <!-- Back Button -->
        <button 
            @click="goBack"
        ...
        >
            <div class="flex flex-col md:flex-row gap-8 items-start">
                <div class="w-32 h-32 rounded-full bg-gradient-to-br from-blue-500 to-purple-500 flex items-center justify-center text-4xl font-bold shadow-2xl shrink-0">
                    {{ freelancer.name[0] }}
                </div>
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-start gap-4 mb-4">
                        <div>
                            <h1 class="text-3xl font-bold mb-2 flex items-center gap-3">
                                {{ freelancer.name }}
                                <div class="px-3 py-1 bg-white/10 rounded-full text-sm font-normal text-white/80 border border-white/10">
        ...
        <div class="mb-8 border-b border-white/10">
            <div class="flex gap-8">
                <button 
                    ...
                    :class="activeTab === 'portfolio' ? 'text-white' : 'text-white/40 hover:text-white/70'"
                >
                    포트폴리오
                    <div 
                ...
                <button 
                    ...
                    class="pb-4 text-lg font-medium transition-colors relative"
                    :class="activeTab === 'reviews' ? 'text-white' : 'text-white/40 hover:text-white/70'"
                >
                    리뷰 / 평판
                    <div 
        ...
        >
            ...
            >
                <div class="aspect-video bg-gradient-to-br from-gray-800 to-gray-700 relative overflow-hidden">
                ...
                <div class="p-6">
                    <h3 class="text-xl font-bold mb-2 group-hover:text-blue-400 transition-colors">{{ item.title }}</h3>
                    <p class="text-white/60 text-sm mb-4 line-clamp-2">{{ item.description }}</p>
                    ...
                    <div class="flex items-center gap-2 text-xs text-white/40 border-t border-white/10 pt-4">
        ...
        <div v-else class="text-center py-20 bg-white/5 rounded-3xl border border-white/5">
            ...
            <p class="text-white/60">아직 작성된 리뷰가 없습니다.</p>
        </div>
    ...
    <div v-else class="min-h-[50vh] flex items-center justify-center text-white/60">
        프리랜서 정보를 찾을 수 없습니다.
    </div>
</template>

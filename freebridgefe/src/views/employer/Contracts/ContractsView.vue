...
const stats = computed(() => [
  ...
  },
  {
    icon: DollarSign,
    label: '총 계약금',
    value: formatCurrency(activeContracts.value.reduce((sum, c) => sum + c.budget, 0)),
  ...
  <div class="max-w-[1400px] mx-auto px-4 md:px-8 py-12 text-white">
    ...
    <div v-if="currentList.length === 0" class="bg-white/5 backdrop-blur-xl rounded-3xl border border-white/10 p-16 text-center">
      ...
      <h3 class="text-2xl font-semibold mb-3">
        {{ activeTab === 'ACTIVE' ? '활성 계약이 없습니다' : '완료된 계약이 없습니다' }}
      </h3>
      <p class="text-white/60">
        {{ activeTab === 'ACTIVE' ? '새로운 프로젝트를 시작해보세요' : '아직 완료된 프로젝트가 없습니다' }}
      </p>
    </div>
        ...
        <div class="flex flex-col lg:flex-row items-start justify-between mb-8 gap-6">
          <div class="flex-1">
            ...
            <div class="text-white/60 flex items-center gap-2">
              ...
              {{ isFreelancer ? `발주자: ${contract.employerName}` : `담당자: ${contract.freelancerName}` }}
            </div>
          </div>
          ...
          >
            ...
            >
              {{ calculateDday(contract.endDate) > 0 ? `-${calculateDday(contract.endDate)}` : calculateDday(contract.endDate) === 0 ? 'Today' : `+${Math.abs(calculateDday(contract.endDate))}` }}
            </div>
          </div>
        </div>

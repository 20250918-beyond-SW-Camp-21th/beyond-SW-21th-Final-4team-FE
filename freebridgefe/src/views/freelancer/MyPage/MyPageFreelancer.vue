    ...
    <main class="flex-1 overflow-y-auto lg:pl-[17.5rem]">
        <div v-if="activeTab === 'dashboard'" class="p-8 max-w-7xl mx-auto space-y-8" v-motion :initial="{ opacity: 0 }" :enter="{ opacity: 1 }">
            ...
            <div class="space-y-4 mb-8">
                <!-- 1. Rate Bump Alert -->
                <div v-if="profile.crmAlerts?.isRateBumpEligible && !hideRateBumpAlert" class="bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-100 rounded-2xl p-4 flex flex-col md:flex-row items-center justify-between gap-4 animate-fade-in-up shadow-[0_18px_40px_-34px_rgba(16,185,129,0.3)]">
                    <div class="flex items-center gap-4">
                        <div class="p-2 bg-emerald-500/20 rounded-full shrink-0">
                            <TrendingUp class="w-6 h-6 text-emerald-400" />
                        </div>
                        <div>
                            <h4 class="text-slate-950 font-bold text-sm">월급 인상 최적기입니다!</h4>
                            <p class="text-slate-600 text-xs mt-1 leading-relaxed">최근 3개 프로젝트에서 좋은 고용주 평가를 받으셨습니다. 이번 기회에 희망 월급을 10~15% 상향 조정해보세요.</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-3 shrink-0">
                        <button @click="activeTab = 'edit'" class="px-4 py-2 bg-emerald-500 hover:bg-emerald-400 text-white text-xs font-bold rounded-lg transition-colors">월급 수정하러 가기</button>
                        <button @click="hideRateBumpAlert = true" class="text-slate-400 hover:text-slate-700 transition-colors p-1"><X class="w-4 h-4" /></button>
                    </div>
                </div>

                <!-- 2. Burnout Alert -->
                <div v-if="profile.crmAlerts?.isBurnoutWarning && !hideBurnoutAlert" class="bg-gradient-to-r from-orange-50 to-rose-50 border border-orange-100 rounded-2xl p-4 flex flex-col md:flex-row items-center justify-between gap-4 animate-fade-in-up shadow-[0_18px_40px_-34px_rgba(251,146,60,0.26)]">
                    <div class="flex items-center gap-4">
                        <div class="p-2 bg-orange-500/20 rounded-full shrink-0">
                            <AlertTriangle class="w-6 h-6 text-orange-400" />
                        </div>
                        <div>
                            <h4 class="text-slate-950 font-bold text-sm">휴식이 필요한 시점입니다.</h4>
                            <p class="text-slate-600 text-xs mt-1 leading-relaxed">최근 프로젝트 일정이 매우 타이트합니다. 컨디션 관리를 위해 잠시 휴식하는 것을 권장합니다.</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-3 shrink-0">
                        <button @click="toggleRestMode" class="px-4 py-2 bg-orange-100 hover:bg-orange-200 text-orange-700 text-xs font-bold rounded-lg border border-orange-200 transition-colors">휴식 모드 전환</button>
                         <button @click="hideBurnoutAlert = true" class="text-slate-400 hover:text-slate-700 transition-colors p-1"><X class="w-4 h-4" /></button>
                    </div>
                </div>

                 <!-- 3. Churn Alert (Encouragement) -->
                 <div v-if="profile.crmAlerts?.isChurnWarning && !hideChurnAlert" class="bg-gradient-to-r from-sky-50 to-cyan-50 border border-sky-100 rounded-2xl p-4 flex flex-col md:flex-row items-center justify-between gap-4 animate-fade-in-up shadow-[0_18px_40px_-34px_rgba(56,189,248,0.24)]">
                    <div class="flex items-center gap-4">
                        <div class="p-2 bg-indigo-500/20 rounded-full shrink-0">
                            <Briefcase class="w-6 h-6 text-indigo-400" />
                        </div>
                        <div>
                            <h4 class="text-slate-950 font-bold text-sm">포기하지 마세요! 딱 맞는 프로젝트가 기다리고 있습니다.</h4>
                            <p class="text-slate-600 text-xs mt-1 leading-relaxed">최근 지원 결과가 아쉬우셨나요? 프리브릿지 AI가 {{ profile.name }}님의 전문성에 꼭 맞는 추천 프로젝트를 준비했습니다.</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-3 shrink-0">
                        <button @click="viewRecommendedProjects" class="px-4 py-2 bg-sky-500 hover:bg-sky-400 text-white text-xs font-bold rounded-lg transition-colors">추천 프로젝트 보기</button>
                         <button @click="hideChurnAlert = true" class="text-slate-400 hover:text-slate-700 transition-colors p-1"><X class="w-4 h-4" /></button>
                    </div>
                </div>
            </div>
